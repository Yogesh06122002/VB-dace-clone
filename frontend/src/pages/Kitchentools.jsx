...
function Kitchentools() {
  ...
  return (
    ...
    <div className='lg:mx-16 mx-4 text-base font-normal pt-16'>
      <div><Link to ='/' className='mr-3 hover:underline'>Home</Link>     /   <Link to ='/products' className='mx-3 hover:underline'>All Products</Link>   / <span className='mx-3'>Kitchen Tools</span></div>
      <div className='mt-3 pb-10 lg:pb-20'>
        <h1 className=' text-3xl lg:text-4xl'>Kitchen Tools</h1>
        <p className='mt-4'>Shop Kitchen Tools exclusively designed by Venkatesh <br className='hidden lg:block' />Bhat.</p>
      </div>
    </div>
    </div>
    <div className='mt-10 mx-4 lg:mx-16 '>
      ...
      <div className=" grid lg:grid-cols-4 grid-cols-2 mt-5 gap-6 lg:gap-10 pb-20 gap-y-14 lg:gap-y-14 lg:pb-28">

        {allProducts && allProducts.filter (product =>  product.category === 'kitchentools').map(product => (
          <Link to={`/product/${product._id}`}>
          <div key={product._id} className='lg:w-[280px] lg:h-[320px] h-48'>
            <div className='relative 'onMouseEnter={()=> setTools(product._id)} onMouseLeave={()=>setTools()}>
              <img src={tools === product._id ? product.images[1] : product.images[0]} alt=""  className='w-full lg:h-[300px] h-44  object-cover bg-[#F5F5F5] p-4 '/>
