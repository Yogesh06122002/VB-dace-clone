  ...
  return (
    ...
    <div className='mx-16 text-base font-normal pt-16'>
      ...
      <div className='mt-3 pb-20'>
        <h1 className=' text-4xl'>Kitchen Mate</h1>
        <p className='mt-4'>Shop Kitchen Mate exclusively designed by Venkatesh <br />Bhat.</p>
      </div>
    </div>
    </div>
        ...
        {allProducts && allProducts.filter (product =>  product.category === 'kitchenmate').map(product => (
          ...
          <div key={product._id} className='w-[280px] h-[320px]'>
            <div className='relative ' onMouseEnter={()=>setMates(product._id)} onMouseLeave={()=>{setMates()}}>
              <img src={mates === product._id ? product.images[1] : product.images[0]} alt=""  className='w-full h-[300px]  object-cover bg-[#F5F5F5] p-4 '/>
