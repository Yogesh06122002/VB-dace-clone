...
function Tableware() {
  ...
  return (
    ...
    <div className='mt-10 mx-4 lg:mx-16 '>
      ...
      <div className=" grid grid-cols-2 lg:grid-cols-4 mt-5 gap-6 lg:gap-10 pb-20 gap-y-14 lg:gap-y-14 lg:pb-28">
        ...
        {allProducts && allProducts.filter (product =>  product.category === 'tableware').map(product => (
          ...
          <div key={product._id} className='lg:w-[280px] h-48 lg:h-[320px]'>
            <div className='relative ' onMouseEnter={() => setTableProduct(product._id)} onMouseLeave={()=>setTableProduct()} >
              <img src={tableProduct === product._id ? product.images[1] : product.images[0]} alt=""  className='w-full lg:h-[300px] h-44  object-cover bg-[#F5F5F5] p-4 '/>
